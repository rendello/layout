...
proptest! {
    ...
    fn reciprocal(s in RE
        ...
        let maps_to_syl = [&maps::BASE_TO_SYL];
        let maps_to_lat = [&maps::BASE_TO_LAT];

        let string_syl = String::new();
        let string_lat = String::new();

        let syl = converter::f(&maps_to_syl[..], &s.as_bytes(), string_syl);
        let lat = converter::f(&maps_to_lat[..], &syl.as_bytes(), string_lat);
